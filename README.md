...
- `GET /entries`, `POST /entries`, `PATCH /entries/:id`, `DELETE /entries/:id` — protegidos por `Authorization: Bearer <accessToken>` **e** por assinatura PRO ativa (`ProActiveGuard`, 403 se `proAtivo` for `false`). `PATCH` recebe o mesmo corpo do `POST` (substitui o registro inteiro, não é patch parcial) e usa `updateMany({ where: { id, userId } })` — mesmo truque do `DELETE`, embute o dono na query em vez de um `findUnique` + checagem separada. `titulo` é opcional — só a tela "bloco de notas" do app (Ideia manual) usa; Gasto e Ideia via gesto mandam sempre `null`
